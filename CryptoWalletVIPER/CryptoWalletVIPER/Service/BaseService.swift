...
open class ServiceTask: ServiceProtocol {
    ...
    public func request(_ httpMethod: HttpMethod, request: Request, onSuccess: @escaping (Data, URLResponse?) -> Void, onError: @escaping (Error) -> Void) {
        ...
        let task = ServiceTask.taskSession.dataTask(with: urlRequest, completionHandler: {(data, response, error) in
            if let error = error {
                onError(error)
                return
            }
            if let responseError = ServiceTask.check(response: response) {
                onError(responseError)
                return
            }
            guard let data = data else {
                onError(ServiceError.recieveNilBody)
                return
            }
    ...
    static func createError(_ code: ServiceError, _ info: [String: Any]?) -> NSError {
        return NSError(domain: "ServiceError", code: code.rawValue, userInfo: info)
    }
